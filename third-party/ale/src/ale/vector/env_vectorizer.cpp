...
BatchResult EnvVectorizer::reset(const std::vector<int>& env_ids, const std::vector<int>& seeds) {
    ...
    std::vector<bool> selected(static_cast<std::size_t>(num_envs_), false);
    for (std::size_t i = 0; i < env_ids.size(); ++i) {
        const int env_id = env_ids[i];
        if (env_id < 0 || env_id >= num_envs_) {
            throw std::out_of_range("reset environment index out of range");
        }
        if (selected[static_cast<std::size_t>(env_id)]) {
            throw std::invalid_argument("reset environment indices must be unique");
        }
        selected[static_cast<std::size_t>(env_id)] = true;
    ...
    actions.reserve(static_cast<std::size_t>(num_envs_));
    auto append_action = [&](int env_id, bool force_reset) {
        Action action;
        action.env_id = env_id;
        action.action_id = 0;
        action.paddle_strength = 1.0f;
        action.force_reset = force_reset;
        action.snapshot_only = !action.force_reset;
        actions.push_back(action);
    };

    // Every selected lane receives a real reset action.  In the normal ordered
    // vector shape we also snapshot every unselected lane so reset() returns a
    // full observation without mutating it.  Async batches only need enough
    // snapshots to complete the requested batch; this preserves ALE's existing
    // batch_size behavior while preventing undersized reset work from hanging.
    for (int env_id = 0; env_id < num_envs_; ++env_id) {
        if (selected[static_cast<std::size_t>(env_id)]) {
            append_action(env_id, true);
        }
    }
    for (int env_id = 0;
         env_id < num_envs_ && actions.size() < static_cast<std::size_t>(batch_size_);
         ++env_id) {
        if (!selected[static_cast<std::size_t>(env_id)]) {
            append_action(env_id, false);
        }
    }
...
void EnvVectorizer::send(const std::vector<Action>& actions) {
    ...
    if (autoreset_mode_ == AutoresetMode::Disabled) {
        for (std::size_t i = 0; i < actions.size(); ++i) {
            const int actual_env_id = last_recv_env_ids_[i];
            if (envs_[actual_env_id]->requires_reset()) {
                throw std::runtime_error(
                    "cannot step terminal Atari lane " + std::to_string(actual_env_id) +
                    " while autoreset_mode is Disabled; reset that lane first"
                );
            }
        }
    }

import os
import subprocess
import sys

import gymnasium as gym


def _is_macos_rosetta_process():
    if sys.platform != "darwin":
        return False
    try:
        result = subprocess.run(
            ["sysctl", "-in", "sysctl.proc_translated"],
            capture_output=True,
            check=False,
            text=True,
        )
    except OSError:
        return False
    return result.stdout.strip() == "1"


def _reject_rosetta_process():
    if _is_macos_rosetta_process():
        raise RuntimeError(
            "env-StableRetro-turbo does not support running under Rosetta. "
            "Use a native arm64 Python and native arm64 libretro cores.",
        )


_reject_rosetta_process()

import env_stableretro_turbo.data
from env_stableretro_turbo.action_tables import ActionTable
from env_stableretro_turbo._retro import Movie
from env_stableretro_turbo._retro import RetroEmulator as NativeRetroEmulator
from env_stableretro_turbo._retro import core_path
from env_stableretro_turbo.enums import Actions, Observations, State
from env_stableretro_turbo.retro_env import RetroEnv

GYMNASIUM_ENV_ID = "EnvStableRetroTurbo-v0"
_GYMNASIUM_VECTOR_ENTRY_POINT = "env_stableretro_turbo:_make_gymnasium_vec_env"

ROOT_DIR = os.path.abspath(os.path.dirname(__file__))
core_path(os.path.join(os.path.dirname(__file__), "cores"))

with open(os.path.join(os.path.dirname(__file__), "VERSION.txt")) as f:
    __version__ = f.read().strip()


__all__ = [
    "Movie",
    "GYMNASIUM_ENV_ID",
    "RetroEmulator",
    "Actions",
    "ActionTable",
    "State",
    "Observations",
    "get_core_path",
    "get_romfile_system",
    "get_system_info",
    "get_fbneo_rom_name",
    "is_fbneo_game",
    "make",
    "RetroEnv",
    "RetroVecEnv",
]

env_stableretro_turbo.data.init_core_info(core_path())


def __getattr__(name):
    if name == "RetroVecEnv":
        from env_stableretro_turbo.vec_env import RetroVecEnv

        return RetroVecEnv
    raise AttributeError(name)


def _make_gymnasium_vec_env(*, game, num_envs=1, **kwargs):
    from env_stableretro_turbo.vec_env import RetroVecEnv

    return RetroVecEnv(game=game, num_envs=num_envs, **kwargs)


def _register_gymnasium_env() -> None:
    existing = gym.registry.get(GYMNASIUM_ENV_ID)
    if existing is None:
        gym.register(
            id=GYMNASIUM_ENV_ID,
            entry_point=None,
            vector_entry_point=_GYMNASIUM_VECTOR_ENTRY_POINT,
        )
        return
    if (
        existing.entry_point is None
        and existing.vector_entry_point == _GYMNASIUM_VECTOR_ENTRY_POINT
        and existing.kwargs == {}
        and existing.max_episode_steps is None
        and existing.additional_wrappers == ()
    ):
        return
    raise gym.error.Error(
        f"Gymnasium environment ID {GYMNASIUM_ENV_ID!r} is already registered "
        "with a conflicting specification",
    )


_register_gymnasium_env()


RetroEmulator = NativeRetroEmulator


def get_core_path(corename):
    """Return the absolute path to the libretro core binary for ``corename``."""
    return os.path.join(core_path(), env_stableretro_turbo.data.EMU_CORES[corename])


def get_romfile_system(rom_path):
    """Infer and return the emulator system name from a ROM file extension."""
    extension = os.path.splitext(rom_path)[1]
    if extension in env_stableretro_turbo.data.EMU_EXTENSIONS:
        return env_stableretro_turbo.data.EMU_EXTENSIONS[extension]
    else:
        raise Exception(f"Unsupported rom type at path: {rom_path}")


def get_system_info(system):
    """Return loaded core metadata for the given system name."""
    if system in env_stableretro_turbo.data.EMU_INFO:
        return env_stableretro_turbo.data.EMU_INFO[system]
    else:
        raise KeyError(f"Unsupported system type: {system}")


def make(
    game,
    state=State.DEFAULT,
    inttype=env_stableretro_turbo.data.Integrations.DEFAULT,
    **kwargs,
):
    """Create and return a ``RetroEnv`` for the requested game and options."""
    try:
        env_stableretro_turbo.data.get_romfile_path(game, inttype)
    except FileNotFoundError:
        if not env_stableretro_turbo.data.get_file_path(game, "rom.sha", inttype):
            raise
        else:
            raise FileNotFoundError(
                f"Game not found: {game}. Did you make sure to import the ROM?",
            )
    return RetroEnv(game, state, inttype=inttype, **kwargs)


def get_fbneo_rom_name(game, inttype=env_stableretro_turbo.data.Integrations.DEFAULT):
    """
    Get the original FBNeo ROM name for a game.
    Returns None if the game doesn't have an original_rom_name in metadata.

    Args:
        game: Game name
        inttype: Integration type (default: DEFAULT)

    Returns:
        str: Original ROM name (e.g., "mk2.zip") or None
    """
    import json

    metadata_path = env_stableretro_turbo.data.get_file_path(game, "metadata.json", inttype)
    if metadata_path and os.path.exists(metadata_path):
        try:
            with open(metadata_path) as f:
                metadata = json.load(f)
                return metadata.get("original_rom_name")
        except (json.JSONDecodeError, OSError):
            pass
    return None


def is_fbneo_game(game, inttype=env_stableretro_turbo.data.Integrations.DEFAULT):
    """
    Check if a game is an FBNeo game.

    Args:
        game: Game name
        inttype: Integration type (default: DEFAULT)

    Returns:
        bool: True if the game is an FBNeo game
    """
    import json

    metadata_path = env_stableretro_turbo.data.get_file_path(game, "metadata.json", inttype)
    if metadata_path and os.path.exists(metadata_path):
        try:
            with open(metadata_path) as f:
                metadata = json.load(f)
                return metadata.get("system") == "FBNeo"
        except (json.JSONDecodeError, OSError):
            pass
    return False


try:
    from farama_notifications import notifications

    if "stable-retro" in notifications and __version__ in notifications["stable-retro"]:
        print(notifications["stable-retro"][__version__], file=sys.stderr)
except Exception:  # nosec
    pass

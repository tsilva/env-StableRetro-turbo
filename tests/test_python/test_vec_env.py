...
def test_stable_retro_threaded_vec_env_same_process():
    pytest.importorskip("stable_baselines3")
    from stable_retro.vec_env import StableRetroThreadedVecEnv

    env = StableRetroThreadedVecEnv(
        [make_counter_env for _ in range(4)],
        num_threads=2,
    )
    try:
        obs = env.reset()
        assert obs.shape == (4, 2, 2, 1)
        assert np.all(obs == 0)

        obs, rewards, dones, infos = env.step(np.asarray([0, 1, 0, 1]))
        assert np.all(obs == 1)
        assert rewards.tolist() == [0.0, 1.0, 0.0, 1.0]
        assert dones.tolist() == [False, False, False, False]
        assert infos == [{"value": 1}] * 4

        obs, rewards, dones, infos = env.step(np.asarray([1, 1, 1, 1]))
        assert np.all(obs == 0)
        assert rewards.tolist() == [1.0, 1.0, 1.0, 1.0]
        assert dones.tolist() == [True, True, True, True]
        assert "terminal_observation" in infos[0]
    finally:
        env.close()

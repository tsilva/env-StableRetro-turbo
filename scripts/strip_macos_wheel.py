#!/usr/bin/env python3
"""Strip Mach-O binaries inside macOS wheels and refresh RECORD."""
...
def is_macho(path: Path) -> bool:
    if path.suffix in MACHO_SUFFIXES:
        return True
    return "helpers" in path.parts and path.is_file()
...
def strip_wheel(wheel: Path, strip_cmd: str, out_dir: Path | None) -> Path:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        total_saved = 0
        for binary in sorted(p for p in root.rglob("*") if is_macho(p)):
            before = binary.stat().st_size
            subprocess.run([strip_cmd, "-x", str(binary)], check=True)
            total_saved += before - binary.stat().st_size

    ...
    try:
        with open(
            env_stableretro_turbo.data.get_file_path(
                game,
                "metadata.json",
                inttype | env_stableretro_turbo.data.Integrations.STABLE,
            ),
        ) as metadata_file:
...
def scan_missing():
    missing = []
    for game in env_stableretro_turbo.data.list_games(env_stableretro_turbo.data.Integrations.ALL):
        if not env_stableretro_turbo.data.get_file_path(
            game,
            "data.json",
            env_stableretro_turbo.data.Integrations.ALL,
        ):
        ...
        if not env_stableretro_turbo.data.list_states(game, env_stableretro_turbo.data.Integrations.ALL):
...
def verify_data(game, inttype, raw=None):
    ...
    path = env_stableretro_turbo.data.get_file_path(game, "data.json", inttype)
    if not path:
        return [], []
    try:
        if not raw:
...
def verify_scenario(game, inttype, scenario="scenario", raw=None, dataraw=None):
    ...
    path = env_stableretro_turbo.data.get_file_path(game, "%s.json" % scenario, inttype)
    if not path:
        return [], []
    try:
        if not raw:
    ...
    try:
        if not dataraw:
            datafile = env_stableretro_turbo.data.get_file_path(
                game,
                "data.json",
                inttype=inttype | env_stableretro_turbo.data.Integrations.STABLE,
            )
...
def verify_default_state(game, inttype, raw=None):
    ...
    path = env_stableretro_turbo.data.get_file_path(game, "metadata.json", inttype)
    if not path:
        return [], []
    try:
        if not raw:
    ...
    if action_sets is None:
        packaged_metadata = (
            Path(env_stableretro_turbo.__file__).parent
            / "data"
            / "stable"
            / game
            / "metadata.json"
    ...
    if not isinstance(action_sets, dict):
        ...
        from env_stableretro_turbo.action_tables import (
        ...
        try:
            buttons = env_stableretro_turbo.get_system_info(system)["buttons"]
        except KeyError:
    ...
    if state not in env_stableretro_turbo.data.list_states(
        game,
        inttype | env_stableretro_turbo.data.Integrations.STABLE,
    ):
...
def verify_hash_collisions():
    ...
    seen_hashes = {}
    for game in env_stableretro_turbo.data.list_games(env_stableretro_turbo.data.Integrations.ALL):
        shafile = env_stableretro_turbo.data.get_file_path(
            game,
            "rom.sha",
            env_stableretro_turbo.data.Integrations.ALL,
        )
...
def verify_genesis(game, inttype):
    ...
    rom = env_stableretro_turbo.data.get_romfile_path(game, inttype=inttype)
...
def verify_extension(game, inttype):
    ...
    rom = os.path.split(env_stableretro_turbo.data.get_romfile_path(game, inttype=inttype))[-1]
    platform = env_stableretro_turbo.data.EMU_EXTENSIONS.get(os.path.splitext(rom)[-1])
...
def verify_rom(game, inttype):
    try:
        env_stableretro_turbo.data.get_romfile_path(game, inttype=inttype)

...
def test_hash(game_name, integration_type):
    errors = env_stableretro_turbo.data.verify_hash(game_name, integration_type)
    assert len(errors) == 0


def test_hash_collisions():
    warnings, errors = env_stableretro_turbo.testing.tools.verify_hash_collisions()
...
def test_rom(game_name, integration_type):
    warnings, errors = env_stableretro_turbo.testing.tools.verify_rom(
        game_name,
        integration_type,
    )
    for file, warning in warnings:

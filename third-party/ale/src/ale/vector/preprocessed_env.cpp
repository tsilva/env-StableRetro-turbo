...
// Coverage-weighted area resampling.  The integer overlap formulation avoids
// platform-dependent floating point drift and exactly preserves constant
// images.  Atari preprocessing only downsizes, matching INTER_AREA's contract.
void resize_area(
    const uint8_t* src,
    int src_height,
    int src_width,
    ...
    if (dst_height > src_height || dst_width > src_width) {
        throw std::invalid_argument("Atari area resize only supports downscaling");
    }

    const std::uint64_t denominator =
        static_cast<std::uint64_t>(src_width) * src_height;
    for (int dy = 0; dy < dst_height; ++dy) {
        const int y_begin = dy * src_height;
        const int y_end = (dy + 1) * src_height;
        const int sy_begin = y_begin / dst_height;
        const int sy_end = (y_end + dst_height - 1) / dst_height;
        for (int dx = 0; dx < dst_width; ++dx) {
            const int x_begin = dx * src_width;
            const int x_end = (dx + 1) * src_width;
            const int sx_begin = x_begin / dst_width;
            const int sx_end = (x_end + dst_width - 1) / dst_width;
            std::uint64_t sums[3] = {0, 0, 0};
            for (int sy = sy_begin; sy < sy_end; ++sy) {
                const int y_overlap = std::max(
                    0,
                    std::min(y_end, (sy + 1) * dst_height) -
                        std::max(y_begin, sy * dst_height)
                );
                for (int sx = sx_begin; sx < sx_end; ++sx) {
                    const int x_overlap = std::max(
                        0,
                        std::min(x_end, (sx + 1) * dst_width) -
                            std::max(x_begin, sx * dst_width)
                    );
                    const std::uint64_t weight =
                        static_cast<std::uint64_t>(x_overlap) * y_overlap;
                    const uint8_t* pixel =
                        src + (static_cast<std::size_t>(sy) * src_width + sx) * channels;
            ...
            uint8_t* output =
                dst + (static_cast<std::size_t>(dy) * dst_width + dx) * channels;
            for (int channel = 0; channel < channels; ++channel) {
                output[channel] = static_cast<uint8_t>(
                    (sums[channel] + denominator / 2) / denominator
                );
            }
        }
    }
...
bool PreprocessedEnv::requires_reset() const {
    return game_over_ || elapsed_steps_ >= max_episode_steps_ ||
        ((life_loss_info_ || episodic_life_) && was_life_lost_);
}
...
void PreprocessedEnv::process_screen() {
    ...
    if (obs_frame_height_ != raw_frame_height_ || obs_frame_width_ != raw_frame_width_) {
        resize_area(
            raw_frames_[0].data(), raw_frame_height_, raw_frame_width_, dest_ptr,
            obs_frame_height_, obs_frame_width_, channels_per_frame_
        );
    } else {
